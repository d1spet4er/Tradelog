...
function Trades() {
  ...
  async function loadJournals() {
    ...
    if (e) {
      setError(`Не удалось загрузить заметки сделок: ${e.message}`)
      return
    }
    const next = {}
  ...
  const keyOptions = useMemo(
    () => keys.map((k) => ({ value: k.id, label: `${displayExchange(k.exchange)} — ${k.label || 'без названия'}` })),
    [keys],
  )
  ...
  async function saveJournal(trade, journal) {
    try {
      ...
      setError(`Не удалось сохранить заметки: ${e.message}`)
    }
  }
  ...
  return (
    <div className="trades-page">
      <div className="trades-page__topline">
        <div>
          <h1>Сделки</h1>
          <p className="trades-page__subtitle">История позиций и торговый дневник</p>
        </div>
      ...
      <section className="trades-sync">
        <div className="section-heading">
          <div>
            <h2>Синхронизация</h2>
            <p>Получаем исполнения с биржи и объединяем их в торговые позиции.</p>
          </div>
        </div>
        ...
        {keys.length > 0 && <>
          <div className="trades-toolbar">
            ...
            <button className="sync-button" onClick={handleSync} disabled={syncing}>
              <span className={syncing ? 'sync-spinner' : ''}>↻</span>
      ...
      <section className="trades-history">
        <div className="history-heading">
          <div>
            <div className="history-title-row">
              ...
              <span className="history-count">{trades.length}</span>
            </div>
            <p>Одна строка — одна позиция. Нажми на сделку, чтобы открыть график, исполнения и заметки.</p>
          </div>
        </div>
        ...
        {trades.length > 0 && <div className="trade-stats">
          <div className="trade-stat-card">
            <span>Сделки</span>
            <strong>{formatNumber(stats.trades)}</strong>
            <small>торговых позиций</small>
          </div>
          <div className="trade-stat-card">
            <span>Исполнения</span>
            <strong>{formatNumber(stats.executions)}</strong>
            <small>ордеров / fills</small>
          </div>
          <div className="trade-stat-card">
            <span>Оборот</span>
            <strong>{formatMoney(stats.volume)}</strong>
            <small>по исполнениям</small>
          </div>
          <div className={`trade-stat-card ${stats.pnl >= 0 ? 'positive-card' : 'negative-card'}`}>
            <span>Реализованный PnL</span>
            <strong>{stats.pnl >= 0 ? '+' : ''}{formatMoney(stats.pnl)}</strong>
            <small>по закрытым позициям</small>
          </div>
        </div>}
